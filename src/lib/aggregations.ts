...
export type WeekBucket = {
  /** ISO date string of the Monday (e.g. "2026-04-07") */
  week: string
  /** Short label for charts (e.g. "4/7") */
  label: string
  /** Aggregated average of values */
  avg: number
...
export function groupByWeek<T>(
  ...
  for (const item of items) {
    ...
    const key = getMonday(new Date(dateFn(item))).toISOString().slice(0, 10)
...
export function trendDelta(buckets: WeekBucket[]): number {
  ...
  const lastAvg = buckets.slice(mid).reduce((s, b) => s + b.avg, 0) / (buckets.length - mid)
